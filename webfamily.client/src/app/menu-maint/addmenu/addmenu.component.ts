...
export class AddmenuComponent implements OnInit, OnDestroy {
  ...
  menu = this.menuService.getAvailableMenus();
  ...
  async addMenuItem(): Promise<void> {
    const menuId = this.menu[this.menuSelectedIndex];
    const title = this.form.value.menuTitle;
    const param = this.form.value.param;
    const success = await this.menuService.addMenuItem(menuId, {
      title: title,
      param: param
    });
    console.log('Add ${menuId} item ${title} result:', success);
    if (success) {
