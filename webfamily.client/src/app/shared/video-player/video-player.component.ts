...
export class VideoPlayerComponent implements OnInit, AfterViewInit {
  ...
  set dataSource(value: VideoSource[]) {
    if (value?.length && value !== this._videos()) {
      /*console.log('Setting new video data source:', value);*/
  ...
  private handlePlayError = (error: Error): void => {
    this.errorMessage.set(`handlePlayError:${error.message}`);
    if (error.name === 'NotAllowedError') {
      this.errorMessage.set('Autoplay prevented by browser. Click play to start the video.');
    }
  };
  ...
  private updateThumbnailPreview(pos: number): void {
    ...
    if (!this.player ||
      ...
      console.log('Duration invalid during hover:', {
        duration: currentDuration,
        readyState: this.player?.readyState,
        networkState: this.player?.networkState
      });

      // Don't update thumbnail with invalid data
      return;
    }
