...
namespace WebFamily.Server.Services
{
    public interface ITubeServices
    {
        ...
        Task<bool> DelWebTube(Guid id);
        Task<bool> DownloadYoutube(YTDownload record);
    }
    public class TubeServices : ITubeServices
    {
        ...
        public async Task<WebTube> AddWebTube(WebTube WebtubeRecord)
        {
            ...
            {
                _uri = new(record.ThumbnailDefaultUrl);
            }
            if (record.ThumbnailStandardUrl.Length > 0)
            {
                _uri = new(record.ThumbnailStandardUrl);
            }
            if (record.ThumbnailMediumUrl.Length > 0)
            {
                _uri = new(record.ThumbnailMediumUrl);
            }
            if (record.ThumbnailHighUrl.Length > 0)
            {
                _uri = new(record.ThumbnailHighUrl);
            }
            if (record.ThumbnailMaxresUrl.Length > 0)
            {
                _uri = new(record.ThumbnailMaxresUrl);
            }
            await DownloadImageAsync(_uri, record.VideoId);
            ...
            for (int i = 0; i < Varray.Length; i++) // for acts as a foreach  
            {
                ...
                {
                    _uriList = new(Varray[i].ThumbNailDefaultUrl);
                }
                if (Varray[i].ThumbNailStandardUrl.Length > 0)
                {
                    _uriList = new(Varray[i].ThumbNailStandardUrl);
                }
                if (Varray[i].ThumbNailMediumUrl.Length > 0)
                {
                    _uriList = new(Varray[i].ThumbNailMediumUrl);
                }
                if (Varray[i].ThumbNailHighUrl.Length > 0)
                {
                    _uriList = new(Varray[i].ThumbNailHighUrl);
                }
                if (Varray[i].ThumbNailMaxresUrl.Length > 0)
                {
                    _uriList = new(Varray[i].ThumbNailMaxresUrl);
                }
                var path = Path.Combine(record.VideoId, WebtubeRecord.VideoListId);
                await DownloadImageAsync(_uriList, path);
            }
